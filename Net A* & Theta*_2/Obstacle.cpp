...
namespace spa2 {
    ...
    Obstacle::Obstacle(const std::vector<Point>& points) : vertex_number(points.size()),
                                                 vertex(points) {
    ...
    Obstacle::Obstacle(const std::vector<std::pair<int, int>>& points) : vertex_number(points.size()) {
        for (int i = 0; i < (int)vertex_number; ++i) {
            vertex.push_back(Point(points[i]));
        }
        *this = Obstacle(vertex);
    }

    int Obstacle::get_xmin() const {
        return x_bounds.first;
    }

    int Obstacle::get_xmax() const {
        return x_bounds.second;
    }

    int Obstacle::get_ymin() const {
        return y_bounds.first;
    }

    int Obstacle::get_ymax() const {
        return y_bounds.second;
    }

    bool Obstacle::do_cross(const Vector& v) const {
    ...
    bool Obstacle::is_point_in(const Point& p) const {
    ...
    void Obstacle::print_to_visual(std::ostream& out) const {
        out << "    <polygon points=" << '"';
